...
if [ -e "$dirfile" -a -f "$dirfile" -a ! -L "$dirfile" ]; then
  ...
  echo "/usr/local/stow/emacs-${version} and not be able to if there is"
  echo "a file already existing there. So for now, to"
  echo "resolve that potential conflict, I am renaming"
